/****************************************************************
 * Name(s)  Patrick Charlton      Jakob Paulson-Palmer          *
 * Box(s):  E189   D217                                         *
...
int main(void) {
    ...
    currTime = mmap(0, BUF_SIZE, PROT_READ | PROT_WRITE,
    ...
    while (numberofWagons < 30) {
        ...
        *currTime = *currTime + 1;
        ...
        if (arrival == 0) {
            ...
            if ((wagonid[numberofWagons] = fork()) == -1) {
                perror ("Fork Failure");
                exit(EXIT_FAILURE);
            }
            else {
